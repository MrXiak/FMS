...
var nowPage = 0, //当前页
    count = 10 //每页显示多少条消息
pageAll = 0; //总页数
var testDataList1 = []; //创建一个存放数据的数组
for (var m = 0; m < 53; m++) { //往数组里放入53条数据
    var filmid = newGuid();
    testDataList1.push(`<tr>
        <td>第` + m + `条消息</td>
        <td>第` + filmid + `条消息</td>
...
pageAll = (testDataList1.length) / count; //计算总页数
var setTable = function () { //数据渲染表格
    ...
    */
    for (var i = 0;
         (i + nowPage * count < (nowPage + 1) * count) && i + nowPage * count < testDataList1.length; i++) { //满足当前数据小于没到当前页的最后一条数据 ，并且当前数据没到最后一条数据
        {
            onePageData.push(testDataList1[i + nowPage * count]);// 这个循环会循环五次  把五条数据放到列表里
        }
    }
